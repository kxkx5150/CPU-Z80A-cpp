...
void z80::z80_init_tables()
{
    for (int i = 0; i < 0x100; i++) {
        ...
        int tmp = sz53p_table[i] = sz53_table[i] | parity_table[i];
    }
    sz53_table[0] |= 0x40;
    sz53p_table[0] |= 0x40;
}
...
void z80::dump(int opcode, int i)
{
    ...
    if (logcheck && filecheck_start <= count && count <= filecheck_end) {
        ...
        char buf1[1000];
        sprintf(buf1, "STEPS=%lu OPCODE=%d tstates=%ld", count, opcode, core->tstates);
        // printf("%s\n", buf1);
        ...
        char buf5[1000];
        sprintf(buf5, "IX=%04X IY=%04X", rIX, rIY);
        // printf("%s\n", buf6);

        char buf6[1000];
        sprintf(buf6, "SP=%04X PC=%04X", rSP, rPC);
        // printf("%s\n", buf7);

        char buf7[1000];
        sprintf(buf7, "I=%04X R=%08X R7=%04X IM=%04X", rI, rR, rR7, rIM);
        // printf("%s\n", buf8);

        char buf8[1000];
        sprintf(buf8, "IFF1=%d IFF2=%d", IFF1, IFF2);
        // printf("%s\n", buf9);
        ...
        if (count < filecheck_end) {
            ...
            sprintf(tbf, "%s %s %s %s %s %s %s %s", buf1, buf2, buf3, buf4, buf5, buf6, buf7, buf8);
            std::string t = tbf;
...
void z80::run()
{
    ...
    switch (opcode) {
        ...
        case 211:
            ...
            break;
        case 219: {
            uint64_t intemp = mem->readMem(rPC++) + (rA << 8);
            rPC &= 0xffff;
            rA = (inb(intemp));
            i += 11;
        } break;
                    ...
                    case 203: {
                        ...
                        ii2 = mem->readMem(rPC++);
                        {
                            switch (ii2) {
                                ...
                                case 126:
                                case 127: {
                                    int tempaddr = mem->readMem(ii);
                                    bit(128, tempaddr);
                                    // if ((tempaddr >> 8) & (0x08 | 0x20))
                                    //     f3 = true;
                                } break;
                                case 128:
        ...
        case 237: {
            ...
            switch (mem->readMem(rPC++)) {
                ...
                case 111: {
                    ...
                    goto EXECrED;
                case 160: {
                    int bytetemp = mem->readMem(HL());
                    mem->writeMem(DE(), bytetemp);
                ...
                case 162:
                    ...
                    goto EXECrED;
                case 163: {
                    int outitemp = mem->readMem(HL());
                    rB           = (dec8(rB));
                    outb(rC, outitemp);
                    int flg = (outitemp & 0x80 ? 0x02 : 0) | sz53_table[rB];
                    setF(flg);
                    setHL((rH << 8 | rL) + 1 & 0xffff);
                    i += 16;
                }
...
void z80::z80_set_irq(int asserted)
{
    irq_pending = asserted;
    if (irq_pending && IFF1) {
        z80_interrupt();
    }
}
...
int z80::inb(int addr)
{
    addr &= 0xff;
    switch (addr) {
        case 0x7e:
            return core->vdp->vdp_get_line();
        case 0x7f:
            return 0;    // core->vdp->vdp_get_x();
        case 0xdc:
        case 0xc0:
        ...
        case 0xbe:
            return core->vdp->vdp_readbyte();
        case 0xbd:
        case 0xbf:
            return core->vdp->vdp_readstatus();
        case 0xde:
            return 0xff;
        case 0xdf:
            return 0xff;
...
void z80::outb(int addr, int val)
{
    val = val | 0;
    addr &= 0xff;
    switch (addr) {
        ...
        case 0xbd:
        case 0xbf:
            core->vdp->vdp_writeaddr(val);
            break;
        case 0xbe:
            core->vdp->vdp_writebyte(val);
            break;
        case 0xde:
            // inputMode = val;
            break;
